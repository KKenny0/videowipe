# VideoWipe 路线图：历史记录与状态更正

Date: 2026-07-26

> **当前执行源**：[fact-baseline.md](fact-baseline.md)。下方原始路线图全文保留，用于保全当时的决策、验收、失败修正与回滚边界；它不再是当前实施清单。

## 2026-07-26 状态更正

- v0.5 的 SDK-first 嵌入门槛已完成。
- A0 只证明音频、软 alpha、进度与 mask 数学；端到端填充质量、时序稳定性、误擦率和用户接受度尚未验证。
- 因此，原文任何将 mask-level proof 表述为“成品感门槛通过”的句子均已撤回，不能作为真实画质或 STTN 成品质量的结论。
- 在事实基线完成并经重新审阅前，不启动默认模型升级、SAM 2、WipePlan 或大型上层产品。
- 当前公开 refs 不提供可直接取回的 Creator Remix 归档分支；原文的归档措辞仅是历史记录，不能被当作可用资产。
- 原文曾引用的 `NEXT_WORK.md` 是非版本化历史规划假设，并非本文的依赖或当前执行源。
...
历史记录曾称 Creator Remix 的 M0–M4 被封存在某个归档；当前公开 refs 不足以验证或取回该归档。翻译工作台、时间线、Codex Workspace 和 M5 不进入当前主线；若以后继续，应作为调用 VideoWipe SDK 的独立上层产品评估。

历史记录曾引用 `plans/NEXT_WORK.md`；该非版本化文件不再是本文依赖，也不决定当前执行。下方相关段落仅保留当时的冲突记录。
...
| 阶段 | 名称 | 状态 | 依赖 | 可独立发布 |
|---|---|---|---|---|
| **A0** | 画质地基（音频 + 软 alpha + 羽化 + 进度） | ✅ 工程完成（质量门槛结论已于 2026-07-26 撤回） | 无 | ✅ |
...
- [x] **历史门槛判定（已撤回，不能推断成品质量）**。STTN + 软 alpha 在 mask 层面达到"成品感"要求（消除硬接缝）。但有两个保留：(1) 端到端帧级肉眼对比因环境 CUDA 问题未完成，建议在有 GPU 的机器上补做；(2) STTN 在复杂动态背景下的填充质量本身（与接缝无关）仍待真人验证——若填充糊，那是 STTN 模型能力问题，非 A0 范围，归 A2。
...
**[已撤回的质量假设] STTN + 软 alpha + 羽化能达到"成品感"门槛。** 若 STTN 在复杂动态背景下本就糊，A0 的工程优化救不回来。验收标准里的人工门槛判定专门用于证伪/证实这条假设。**这是整条路线 load-bearing 的假设，必须在 A0 完成时立即验证。**
...
## 与 NEXT_WORK.md 的历史冲突记录（非当前依赖）

非版本化的 `NEXT_WORK.md` 曾列出三条与本路线冲突的假设。C1 完成后，这里只保留仍有效的约束：
...
整条路线的发布门禁（非单阶段）：
- [ ] A0 的历史人工门槛判定曾被标记为通过；该结论已撤回，不能作为发布门禁证据
