...
| 无 remove 帧平均误擦面积 | ≤ 0.020 | 0.0317 | **未达；批准为 v1 范围例外** |
| 任一无 remove 帧误擦面积 | ≤ 0.030 | 0.0634 | **未达；批准为 v1 范围例外** |
...
### Phase A 质量验收决定（2026-07-28）

产品侧批准将上述两项未达指标作为 **WipePlan v1 已知时序分辨率例外**，不要求在 Phase A 内通过增加采样密度、逐帧检测或 mask propagation 来消除。理由不是指标已经通过，而是第 361 帧的状态没有出现在任何采样证据中；在“固定采样数 + 最近样本状态插值”的 v1 契约下，执行器无法从前后均为 presence 的观测中推导出中间短空窗。
