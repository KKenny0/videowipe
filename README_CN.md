...
![清理工作区：左侧查看视频与实际处理范围，右侧选择清理目标和设置保护区域](pics/web-ui/workspace-review.jpg)

*开发版真实界面记录。橙色表示实际处理范围；截图用于介绍操作，不代表最新画质已通过验收。*

1. **选择要擦的内容。** 查看目标卡片，不勾选需要保留的内容。
2. **先看一小段效果。** 使用推荐的 3 秒窗口，同步对比原片和结果；必要时调整目标或添加保护区。
3. **处理并检查全片。** 按建议位置复查，再下载结果。一段试擦满意，不代表整片都没有问题。

修改会自动保存，相同设置的试擦可以复用。[详细说明](docs/cleanup-workspace_CN.md)介绍任务恢复、保护区域、缓存限制和 SDK 用法。
...
例如，`videowipe clean input.mp4 -o result/` 会通过 `WipeEngine` 执行以下路径：

```text
输入视频 → 检测文字 → 生成 WipePlan → 按帧应用移除 mask
         → STTN 修复 → 写出保留原音轨的 MP4
```

**默认 CLI 命令会直接继续修复，不会停下来等待审阅。** 可按需要选择审阅方式或提供移除区域：

| 输入 / 选项 | 执行路径 |
|-------------|----------|
| `--preview` | 检测并保存计划及预览产物后停止，不加载修复模型。可编辑计划 JSON 后再执行。 |
| `--plan plan/wipe_plan.json` | 读取并校验已审阅的计划，然后修复，不重新自动检测。 |
| `--mask mask.png` | 跳过自动检测和规划，按提供的 mask 修复。与 `--plan` 互斥。 |

若要在 CLI 中交互选择后再修复，使用 `--confirm`。本地网页界面提供上文的“选择目标 → 短片试跑 → 完整结果”流程。
