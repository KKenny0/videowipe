...
```bash
# Apple Silicon（M1/M2/M3/M4）：自动使用 MPS
...
视频、目标卡片和下一步操作放在同一工作区。选片后查看自动推荐的试擦，
确认后原地播放、检查和下载全片。目标卡片使用实际出现帧，暂停时可切换原片与结果。
...
可选的 `on_candidates` 在候选证据保存后、时间细化前收到独立元数据快照，可用于提前展示审阅界面；执行仍需等待 `plan()` 返回。回调异常会向上传播。
