...
本地网页检测完成后，可以选择起始秒数，试擦 3 秒或 5 秒；临近片尾会自动缩短。
播放器上半幅是原片，下半幅是修复结果，两者同步播放，并保留对应区间的音频。
调整目标、区域框或时间后，旧试擦会隐藏，需要重新试擦；满意后点 **Clean video** 处理全片。
试擦失败可以直接重试，不必重新上传。刷新网页可恢复当前任务，但重启服务不会恢复内存中的任务。

试擦目前仅支持内置 STTN，沿用全片分段上下文与掩码；它只代表选中区间的效果。
SDK 可通过 `WipeRequest(trial_range=(start_frame, end_frame), ...)` 生成同样的对比视频，
